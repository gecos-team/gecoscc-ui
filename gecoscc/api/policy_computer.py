...
class ComputerPolicies(ComputerResource):
    ...
    def get_policies_and_objects_users(self, type_policies, db):
        ...
        for type_policy in type_policies.keys():
            ...
            if not policy_users:
                continue
            if type_policy == 'user_shared_folders_res':
                policies.append('storage_can_view')
            else:
    ...
    def get_policies_applied_to_users(self, computer_node, db):
        ...
        for policy_slug in type_policies.keys():
            ...
            if not policy_users:
                continue
            if policy_slug in ('user_shared_folders_res'):
                policy = self.get_element(policies, 'slug', self.emitters_policy_slug(policy_slug))
            else:
        ...
        for type_policy in type_policies.keys():
                for policy_slug in type_policies.get(type_policy).keys():
                    ...
                    if not policy_updated_by:
                        continue
                    if policy_slug in ('printers_res', 'user_shared_folders_res', 'software_sources_res'):
